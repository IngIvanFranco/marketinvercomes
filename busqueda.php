...
<article id="products" class="contenedorproductos">
   ...
   $empieza = ($pagina-1) * $por_pagina;
        ...
        //get rows query
        $query = $db->query("SELECT * FROM products WHERE status = '1'  
        AND (products.name LIKE '%$q%' OR products.description LIKE '%$q%') 
...
echo "<a href='busqueda.php?q=$q&pagina=1' class='pagesq'>"."<i class='icon-left'></i>"."</a>";

for($i=1; $i<=$total_paginas; $i++){
   echo "<a href='busqueda.php?q=$q&pagina=".$i."' class='numpage'>".$i."</a>";
}

echo "<a href='busqueda.php?q=$q&pagina=$total_paginas' class='pagesq'>"."<i class='icon-right'></i>"."</a>";
